...
For each of the 13 target variables listed in Table~\ref{tab:sensors}, the data was randomly partitioned into a 90:10 training/testing split. Using Julia's MLJ (Machine Learning Julia) package, a variety of machine learning models compatible with the regression task were identified \cite{MLJ1}. This selection of models included linear regression (and it's variations), Gaussian process regression, shallow neural networks, and decision tree-based models. Out of this set, the Random Forest Regressor (RFR) model from \cite{decision-trees} was selected for use in this study due to its relatively quick training and inference times. Random forests are an ensembling technique based on bagged predictions of individual decision tree regressors trained using the classification and regression trees (CART) algorithm. Each individual tree in a RFR is trained on a random subset of features \textit{and} a random subset of training records \cite{decision-trees, random-forest}.
